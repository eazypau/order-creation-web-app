		...
		<aside className="absolute right-0 top-0 z-20 lg:w-4/12 xl:w-1/4 2xl:w-3/12 px-5 py-5 lg:py-10 bg-white h-full">
			<div className="flex justify-between mb-3 lg:mb-5">
				<div className="flex items-center gap-1 lg:gap-2">
					...
					<button onClick={toggleSidebarFunc} className="w-5 lg:w-fit">
						<ChevronDoubleRightIcon className="w-5 lg:w-7 hover:opacity-70" />
					</button>
					<h5 className="sidebar-heading">{inputValue.orderNumber}</h5>
				</div>
				<p className={"py-1 px-4 text-sm lg:text-base rounded-lg " + (inputValue.status === "unfulfill" ? "bg-amber-200" : "bg-green-300")}>{inputValue.status}</p>
			</div>
			<div>
				<form>
					<div className="mb-5">
						<label htmlFor="customerName" className="pl-1 font-semibold text-sm lg:font-base">Customer name:</label>
						<InputField
							hasBorder={true}
							type="text"
							name="customerName"
					...
					<label className="pl-1 font-semibold text-sm lg:font-base">List of items</label>
					<div className="mb-5 space-y-2">
						{inputValue.items.map((item, index) => (
							<div key={inputValue.orderNumber + "-" + index} className="flex gap-2">
								<SelectBox value={item.itemName} onChange={(e) => handleSelectChange(e, index)} />
								<InputField
									hasBorder
									customWidth="w-24"
									type="number"
